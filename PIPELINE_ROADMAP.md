...
- Estado: completada v1 con depth ONNX y normal procedural.
...
- Resultado: `ue5_depth` exporta `depth.png` con Depth Anything v2 Small ONNX `model_q4f16.onnx` cacheado fuera del repo.
- Resultado: `ue5_normal_map` exporta `normal.png` con fallback procedural por luminancia; no se presenta como IA.
- Protegido: no se instalan modelos 3D ni Stable Zero123; los modelos no se empaquetan dentro del EXE.

### Fase 6 - Calidad automatica

- Estado: completada en modo opcional con backend PyIQA externo validado.
...
- Resultado: `quality_pyiqa` queda como scorer opcional; usa `pyiqa` local si existe, `.venv_pyiqa` si esta presente, o `IMAGE_ENHANCER_PYIQA_PYTHON` para un entorno externo.
- Resultado: `score` solo aparece cuando `quality_pyiqa` esta activo y el backend responde.
- Evidencia: backend externo dedicado `.venv_pyiqa` validado con `pyiqa==0.1.15.post2` y metrica `niqe`.
- Protegido: no hay autoajustes del pipeline sin evidencia.

### Cierre distribuible del nucleo

- Estado: completado tras repriorizar ComfyUI como opcional.
- Resultado: EXE `onedir/windowed` reconstruido en `image_enhancer\image_enhancer\dist\ImageEnhancer-Unova`.
- Resultado: el artefacto pasa `--smoke`, `--smoke-cuda`, `--smoke-modules` y `--smoke-ue5-depth-normal`.
- Resultado: el artefacto no incluye `.venv_pyiqa`, `pyiqa`, `torch` ni el modelo cacheado de Depth Anything; esos recursos siguen externos.
- Nota: el paquete completo ronda 3 GB por ONNX Runtime, rembg, skimage y DLLs NVIDIA.
...
Imagen de entrada
  ...
  -> Score PyIQA opcional si `quality_pyiqa` esta activo y existe backend externo/local
  -> Refinado manual opcional
  -> Export PNG/JPG o UE5 Texture Set (`albedo.png`, `depth.png`, `normal.png`, manifest segun modulos activos)
```
...
| `ue5_texture_set` | UE5 Texture Tools | listo | exporta carpeta UE5 con `albedo.png`, mapas activos y manifest |
| `quality_pyiqa` | Calidad automatica | listo si existe `.venv_pyiqa` o backend configurado | score visible solo si el backend responde |
| `ue5_depth` | UE5 Texture Tools | listo con cache externa | genera `depth.png` con Depth Anything v2 Small ONNX |
| `ue5_normal_map` | UE5 Texture Tools | listo con fallback procedural | genera `normal.png` por luminancia; `IMAGE_ENHANCER_UE5_NORMAL_ONNX` queda como ruta futura |
...
| `creative_comfyui` | Creativa | opcional externo | backend ComfyUI configurable con workflows JSON, reference/control image y retorno no bloqueante |
| `production_batch` | Produccion | listo | batch por carpeta, presets guardables, export profiles, historial y reporte final |
...
- Orden:
  ...
  - Integrar `ue5_depth` cuando se valide un modelo ONNX viable. Hecho con Depth Anything v2 Small ONNX `model_q4f16.onnx`.
  - Generar `depth.png`. Hecho cuando `ue5_depth` esta activo.
  - Agregar `normal.png`. Hecho con fallback procedural por luminancia; `IMAGE_ENHANCER_UE5_NORMAL_ONNX` queda como ruta futura para ONNX real.
- No hacer todavia:
  - No integrar Stable Zero123.
  - No meter modelos 3D pesados.
- Exito:
  - Exportar una carpeta con nombres estables para UE5. Hecho.
  - La app sigue funcionando aunque falte el modelo depth. Hecho con fallback/warnings.
  - `--smoke-ue5-depth-normal` valida `depth.png`, `normal.png` y manifest. Hecho.
- Prioridad: muy alta por diferenciacion.

### Fase 6 - Calidad automatica

- Objetivo: que la app mida si una imagen mejoro o empeoro.
- Estado: completada en modo opcional con backend externo validado.
- Orden:
  - Registrar metricas simples sin dependencia: tamano, alfa, modo, warnings. Hecho.
  - Evaluar PyIQA como modulo opcional. Hecho sin volverlo dependencia obligatoria del runtime base.
  - Mostrar `score` solo si esta disponible. Hecho.
  - Validar backend real. Hecho con `.venv_pyiqa` y `--smoke-quality-pyiqa`.
- No hacer todavia:
...
- Exito:
  ...
  - PyIQA puede calcular `niqe` desde runtime externo dedicado sin tocar `requirements.txt`. Hecho.
...
- Orden:
  - Zero-DCE o DeepWB primero, por impacto y UI simple. Registrados como candidatos ONNX por variable de entorno; fallback validado sin modelos instalados.
...
- Orden:
  ...
  - Smoke dedicado de produccion. Hecho.
- Exito:
...
- Estado: opcional avanzado, no prioritario para el nucleo; plantillas JSON validas; backend local sigue sin verificarse end-to-end porque `--smoke-comfyui` falla con `WinError 10061` al no haber servidor ComfyUI escuchando en `http://127.0.0.1:8188`.
- Orden:
...
- Prioridad: opcional; no compite con el nucleo propio.

### Fase 11 - Video

- Objetivo: procesamiento por frames sin romper el producto de imagenes.
- Estado: completada como sprint aislado.
- Orden:
  - Sprint separado. Hecho.
  - Pipeline de frames. Hecho en `video_pipeline.py`.
  - Progreso largo y cancelacion fuerte. Hechos en el runner aislado.
  - Export de video. Hecho via `ffmpeg`.
- No hacer todavia:
  - No mezclar RIFE/BasicVSR++ con el pipeline de imagen actual.
- Nota:
  - RIFE y BasicVSR++ quedan evaluados como backends externos por ruta/script, no integrados al runtime base.
- Prioridad: futura.
...
1. Profundizar Fase 5 solo si se quiere reemplazar el normal procedural por un ONNX real en `IMAGE_ENHANCER_UE5_NORMAL_ONNX`.
2. Profundizar Fase 8: validar Zero-DCE/DeepWB/SwinIR/NAFNet ONNX solo cuando haya modelo local viable.
3. Profundizar Fase 7: CodeFormer ONNX solo si hay ruta estable sin PyTorch en el EXE.
4. Usar ComfyUI solo como backend creativo opcional si se necesita img2img/IP-Adapter/ControlNet.
5. Evaluar RIFE/BasicVSR++ reales solo dentro del sprint de video aislado.

Motivo:

- El nucleo propio ya tiene version distribuible validada; el siguiente salto debe reforzar modulos propios, no depender de ComfyUI.
- UE5 Texture Tools ya diferencia el producto con `albedo.png`, `depth.png` y `normal.png`; el unico matiz es que `normal.png` sigue siendo fallback procedural, no IA.
- Los candidatos ONNX solo deben entrar cuando haya modelo probado, smoke y fallback.
- ComfyUI no define la utilidad de la app: queda como conector creativo para workflows que no pertenecen al producto base.
...
- Validar.
- Smoke test.
- Fallback cooperativo si el backend o modelo no esta disponible.
