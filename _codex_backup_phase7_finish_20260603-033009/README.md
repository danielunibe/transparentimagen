...
- Exportar `UE5 Texture Set` con carpeta estable, `albedo.png` y `texture_manifest.json`.
...
22. Usa `UE5 Set` para exportar una carpeta `<asset>_UE5_Texture_Set` con `albedo.png` y `texture_manifest.json`. `depth.png` y `normal.png` quedan omitidos con warning hasta validar modelos ONNX propios.
...
| `portrait_gfpgan_external` | `No instalado` | Ruta candidata para GFPGAN externo sin PyTorch en el EXE |
| `ue5_texture_set` | `Listo` | Exporta carpeta UE5 con `albedo.png`, manifest y fallback depth/normal |
| `ue5_depth` | `No instalado` | Reservado para Depth Anything v2 |
| `ue5_normal_map` | `No instalado` | Reservado para normal map IA |
| `quality_pyiqa` | `Fallback` o `Listo` | Scoring opcional via `pyiqa` local o `IMAGE_ENHANCER_PYIQA_PYTHON` |
...
| `restoration_swinir` | `No instalado` | Candidato futuro SwinIR/HAT/BSRGAN |
...
- Fase 5 v1: UE5 Texture Set con `albedo.png` y manifest completado; depth/normal quedan pendientes de modelo validado.
- Fase 6: metricas simples en resultado/UI/export completadas; `score` aparece solo si `quality_pyiqa` esta activo y el backend responde.
- Fase 7 base: detector facial local + CodeFormer externo con fallback no bloqueante completado.
- Fase 8 base: `color_local_restore` completado; Zero-DCE/DeepWB/SwinIR quedan como candidatos.
- Fase 9: produccion base completada con presets guardables, export profiles, batch por carpeta, historial y reporte final por lote.
- Fase 10 base: ComfyUI externo opcional integrado con smoke API, workflow JSON y retorno no bloqueante.
- Fase 11: separado en `..\..\VIDEO_SPRINT.md`; no entra en el runtime actual.

Siguiente orden recomendado:

1. Fase 5.2/5.3: validar modelos ONNX viables para `depth.png` y `normal.png`.
2. Profundizar Fase 10 con workflows reales de img2img/IP-Adapter/ControlNet en ComfyUI.
...
- Placeholders soportados en el workflow:
  ...
  - `__COMFY_IMAGE__`
  - `__OUTPUT_PREFIX__`
- La app sube la imagen, encola el workflow y espera el resultado sin bloquear la UI.
- Hay plantillas base en `workflows\comfyui\basic_txt2img_template.json` y `workflows\comfyui\basic_img2img_template.json`.
- IP-Adapter, img2img y ControlNet deben entrar como workflows externos de ComfyUI, no como runtime propio de la app.
