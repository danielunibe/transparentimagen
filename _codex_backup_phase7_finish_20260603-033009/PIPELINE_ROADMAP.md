...
| `portrait_gfpgan_external` | Retratos | no instalado | candidato externo por `IMAGE_ENHANCER_GFPGAN_SCRIPT` |
...
| `restoration_swinir` | Restauracion | no instalado | candidato futuro SwinIR/HAT/BSRGAN segun ONNX viable |
...
- Estado: base completada con fallback.
- Orden:
  ...
  - Evaluar GFPGAN solo si no exige PyTorch dentro del EXE. Registrado como externo por `IMAGE_ENHANCER_GFPGAN_SCRIPT`, no instalado.
  - Agregar detector facial solo si desbloquea recorte/mezcla real. Hecho con OpenCV Haar cascade para crop de CodeFormer.
- No hacer todavia:
  - No meter PyTorch al runtime principal.
- Exito:
  - Retratos mejoran como capa opcional y con fallback; si CodeFormer falla/no esta configurado, el render final no se bloquea.
- Prioridad: media.
...
- Orden:
  ...
  - Despues SwinIR/HAT/BSRGAN segun modelo ONNX viable. Registrado como candidato futuro.
  - NAFNet solo si el caso de ruido lo justifica. Pendiente.
- Exito:
  - Cada modulo se activa/desactiva y no bloquea el render final si falla. Hecho para `color_local_restore`.
- Prioridad: media.
...
- Estado: completada base externa; backend local no verificado porque ComfyUI no estaba levantado durante el smoke.
- Orden:
  ...
  - IP-Adapter / img2img solo via backend externo. Preparado por workflow JSON y plantillas base; pendiente de workflow real validado.
  - ControlNet despues. Preparado por workflow JSON; pendiente de workflow real validado.
- No hacer todavia:
  - No hacer Stable Diffusion core dentro de la app base.
- Exito:
  - La app puede delegar a ComfyUI local si existe, sin depender de el para funcionar. Hecho a nivel de integracion.
- Prioridad: futura.
...
2. Profundizar Fase 10: levantar ComfyUI real y validar workflows concretos de img2img/IP-Adapter/ControlNet.
3. Profundizar Fase 5: elegir modelos ONNX viables para `ue5_depth`/`ue5_normal_map`.
4. Profundizar Fase 8: validar Zero-DCE/DeepWB ONNX solo cuando haya modelo local viable.
5. Profundizar Fase 7: CodeFormer ONNX o GFPGAN externo solo si hay ruta estable sin PyTorch en el EXE.
