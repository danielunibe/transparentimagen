...
- Estado: completada en modo externo configurable; plantillas JSON validas; backend local sigue sin verificarse end-to-end porque `--smoke-comfyui` falla con `WinError 10061` al no haber servidor ComfyUI escuchando en `http://127.0.0.1:8188`.
- Orden:
...
1. Levantar un ComfyUI real y validar end-to-end alguno de los workflows externos ya preparados.
2. Profundizar Fase 5 solo si se quiere reemplazar el normal procedural por un ONNX real en `IMAGE_ENHANCER_UE5_NORMAL_ONNX`.
3. Profundizar Fase 8: validar Zero-DCE/DeepWB/SwinIR/NAFNet ONNX solo cuando haya modelo local viable.
4. Profundizar Fase 7: CodeFormer ONNX solo si hay ruta estable sin PyTorch en el EXE.
