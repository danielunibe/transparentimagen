...
- [~] Validar ComfyUI local como backend externo opcional no prioritario: integracion y plantillas JSON validas; `--smoke-comfyui` falla con `WinError 10061` porque no hay servidor ComfyUI real escuchando en `http://127.0.0.1:8188`.
...
1. [x] Cerrar version distribuible: EXE reconstruido y smokes del nucleo propio validados.
...
7. [ ] Siguiente de producto propio: validar un ONNX real para `IMAGE_ENHANCER_UE5_NORMAL_ONNX` si se quiere reemplazar el normal procedural.
8. [ ] Siguiente opcional creativo: instalar/levantar ComfyUI externo y apuntar un workflow real desde la UI, solo si se necesita img2img/IP-Adapter/ControlNet.

Motivo:

- Fase 9 ya esta operativa; el valor principal no depende de ComfyUI sino de un flujo guiado local para recorte, upscale, calidad, batch y export UE5.
...
- ComfyUI queda como backend creativo avanzado y opcional; no debe bloquear el roadmap del nucleo.
- EXE reconstruido en `image_enhancer\image_enhancer\dist\ImageEnhancer-Unova\ImageEnhancer-Unova.exe`; no incluye `.venv_pyiqa`, `pyiqa`, `torch` ni modelos cacheados.
