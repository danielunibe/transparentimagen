...
- Face recovery: integracion externa opcional con CodeFormer oficial via `inference_codeformer.py`, con detector facial local OpenCV para crop/mezcla si esta activo.
...
- `ue5_depth`, `ue5_normal_map` y `quality_pyiqa` siguen sin dependencia obligatoria; no instalar modelos/scoring sin sprint propio.
...
- Impacto: Fase 3 CUDA real, Fase 4 segmentacion premium, Fase 5 UE5 Texture Set v1 y Fase 6 metricas simples ya quedaron completadas en base; depth/normal/scoring real quedan como profundizacion.
- Estado: confirmado.
...
- Impacto: `--smoke-portrait` y `--smoke-color-restore` validan fallback; `portrait_face_detector` y `color_local_restore` pueden activarse desde `MODULOS IA`.
...
- Impacto: `comfyui_client.py` maneja upload, cola, polling y cancelacion cooperativa; `--smoke-comfyui` valida la API local.
- Estado: confirmado para Fase 10 base.
