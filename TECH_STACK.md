...
## 0. Asteria frontend oficial

- Ruta: `C:\Desarrollos DEV daniel\transparentimagen\asteria`.
- Frontend: Vite + React 19 + TypeScript.
- Desktop: Tauri v2.
- Persistencia desktop: SQLite via `tauri-plugin-sql`.
- Persistencia Asteria: SQLite es la unica fuente de verdad; almacenamiento web solo se consulta desde la migracion antigua.
- Carpetas Asteria: rutas Windows persistidas, Tauri Dialog y escaneo Rust de solo lectura.
- Estado de ventana: `tauri-plugin-window-state`.
- Dev server: `127.0.0.1:5174`.
- Motivo del puerto: `5173` estaba ocupado por Julia durante la migracion.
- Estado: migracion base implementada el 2026-06-07.
- No cambiar sin permiso: no volver a Next.js, no tocar `image_enhancer\image_enhancer` para cambios de Asteria.
- Asteria no soporta modo web u online; Vite es infraestructura interna de Tauri.
...
- Export imagen compatible: perfil `SVG compatible` con SVG que incrusta PNG para preservar fidelidad y transparencia sin dependencias nuevas.
...
- Backend creativo opcional: ComfyUI local via HTTP + workflow JSON externo; no es requisito del nucleo.
...
| `.venv\Scripts\python.exe image_enhancer.py --smoke-svg-export` | Validar export SVG compatible con PNG incrustado | nuevo |
...
- ComfyUI queda desacoplado del runtime base; se consume por API local y workflow JSON sin meter Stable Diffusion core dentro de la app, y no bloquea el producto si no esta instalado.
...
- Fecha: 2026-06-03
- Decision: ComfyUI queda como conector creativo opcional, no como dependencia ni prioridad del nucleo.
- Motivo: la app se diferencia como herramienta guiada de produccion local: recorte con alfa, refinado, Real-ESRGAN, UE5 Texture Set, metricas, presets y batch.
- Impacto: el siguiente roadmap prioriza modulos propios y EXE validado; ComfyUI solo se usa si se necesita img2img/IP-Adapter/ControlNet.
- Estado: confirmado.
- No cambiar sin permiso: no bloquear recorte, upscale, batch, calidad ni export UE5 por falta de ComfyUI.
