...
class ImageEnhancerApp:
    ...
    def _deactivate_module(self, module_id: str):
        ...
        if module_id == "portrait_face_detector" and not MODULE_REGISTRY.is_active("portrait_codeformer"):
            self.do_face_recovery.set(False)
        self._refresh_module_cards()
        self._set_status(f"Modulo {module_id}: {state}", self._module_status_color(state))
    ...
    def _collect_processing_options(self, render_mode: str) -> ProcessingOptions:
        ...
        return ProcessingOptions(
            ...
            do_face_recovery=self.do_face_recovery.get() or "portrait_codeformer" in active_modules,
    ...
    def _restore_processing_buttons(self):
        ...
        preview_bg = ACTION_LOW if self.input_image else PANEL2
        self.preview_btn.configure(state=state, bg=preview_bg, fg=TEXT if self.input_image else TEXT_DIM, text="Preview 25%")
        if self._segmentation_compare_btn:
