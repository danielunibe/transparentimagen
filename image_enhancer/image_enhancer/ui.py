...
class ImageEnhancerApp:
    ...
    def _build_comfyui_panel(self, parent):
        ...
        tk.Label(
            parent,
            text="Backend externo opcional. Soporta workflow JSON con imagen principal, reference/control image y placeholders configurables.",
            font=(FONT_UI, 8),
            fg=TEXT_DIM,
            bg=PANEL,
            wraplength=250,
    ...
    def _capture_preset_payload(self) -> dict:
        ...
        active_modules = [
            module_id
            for module_id in options.active_modules
            if module_id
            not in {
                "production_batch",
                "creative_comfyui",
        ...
        return {
            ...
            "export_profile_id": self._export_profile_id() if hasattr(self, "export_profile_var") else "png_transparent",
        }
    ...
    def _apply_saved_preset_payload(self, payload: dict):
        ...
        export_profile_id = str(payload.get("export_profile_id", "")).strip()
        if not export_profile_id:
            legacy_label = str(payload.get("export_profile_label", "")).strip()
            export_profile_id = export_profile_by_label().get(legacy_label, "")
        if export_profile_id and hasattr(self, "export_profile_var"):
            profile_label = next(
                (label for label, profile_id in export_profile_by_label().items() if profile_id == export_profile_id),
                "",
            )
            if profile_label:
                self.export_profile_var.set(profile_label)
        self._refresh_module_cards()
        self._refresh_action_chips()
    ...
    def _export_current_with_profile(self):
        ...
        record_processing_history(
            ...
            ProcessingResult(
                ...
                render_mode="export-profile",
                provider=self._last_provider or "",
                warnings=[],
                metrics=self._last_metrics or {},
            ),
            kind="export-profile",
            extra={"outputs": [str(path) for path in outputs], "profile": self._export_profile_id()},
        )
        self._refresh_history_panel()
    ...
    def _open_edge_refiner(self):
        ...
        def apply_refined(refined: Image.Image):
            ...
            self._last_provider = "manual refine"
            self._refresh_quality_panel(self._last_metrics)
            self._render_previews()
    ...
    def _save_output(self):
        ...
        try:
            ...
            if lower_path.endswith((".jpg", ".jpeg")):
                ...
                save_image_variant(self.output_image, Path(path), "svg")
            else:
