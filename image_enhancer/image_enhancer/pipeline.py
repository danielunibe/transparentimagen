...
from models import (
    ...
    ZERO_DCE_ENV_ONNX,
    cv2,
    get_depth_anything_session,
    get_realesrgan_session,
    get_rembg_session,
    np,
    resolve_pyiqa_python_path,
)
...
def maybe_collect_quality_score(
    ...
    *,
    ...
    external_python = resolve_pyiqa_python_path()
...
def _normalize_float_to_u8(values) -> Image.Image:
    ...
    if not np.isfinite(min_value) or not np.isfinite(max_value) or max_value <= min_value:
        ...
        normalized = ((arr - min_value) * (255.0 / (max_value - min_value))).clip(0, 255).astype(np.uint8)
    return Image.fromarray(normalized, mode="L")


def generate_depth_anything_map(image: Image.Image) -> Image.Image:
    if not ORT_AVAILABLE or not NUMPY_AVAILABLE:
        raise RuntimeError("Depth Anything requiere onnxruntime y numpy.")

    session = get_depth_anything_session()
    input_meta = session.get_inputs()[0]
    input_name = input_meta.name
    size = 518
    rgb = ImageOps.exif_transpose(image.convert("RGB")).resize((size, size), Image.Resampling.BICUBIC)
    arr = np.asarray(rgb, dtype=np.float32) / 255.0
    mean = np.array([0.485, 0.456, 0.406], dtype=np.float32)
    std = np.array([0.229, 0.224, 0.225], dtype=np.float32)
    arr = (arr - mean) / std
    batch = arr.transpose(2, 0, 1)[None, :, :, :].astype(np.float32)
    output = session.run(None, {input_name: batch})[0]
    depth = np.asarray(output)
    while depth.ndim > 2:
        depth = depth[0]
    return _normalize_float_to_u8(depth).resize(image.size, Image.Resampling.BICUBIC)


def generate_luminance_normal_map(image: Image.Image, strength: float = 2.5) -> Image.Image:
    if not NUMPY_AVAILABLE:
        raise RuntimeError("normal.png procedural requiere numpy.")

    gray = ImageOps.autocontrast(image.convert("RGB").convert("L"))
    ...
    normal_z = np.ones_like(height, dtype=np.float32)
    ...
    return Image.fromarray((normal * 255.0).clip(0, 255).astype(np.uint8), mode="RGB")
...
def export_ue5_texture_set(
    ...
    *,
    ...
    sources = {"albedo": "processed_rgba"}

    depth_status = MODULE_REGISTRY.status("ue5_depth")
    normal_status = MODULE_REGISTRY.status("ue5_normal_map")
    if depth_status == "Activo":
        try:
            depth_path = export_folder / "depth.png"
            generate_depth_anything_map(image).save(depth_path, format="PNG", optimize=True)
            export.files["depth"] = depth_path
            sources["depth"] = "Depth Anything v2 Small ONNX q4f16, relative normalized 8-bit"
        except Exception as exc:
            export.warnings.append(f"depth.png omitido: {exc}")
    else:
        export.warnings.append("depth.png omitido: ue5_depth aun no tiene modelo ONNX validado.")

    if normal_status == "Activo":
        try:
            normal_path = export_folder / "normal.png"
            generate_luminance_normal_map(image).save(normal_path, format="PNG", optimize=True)
            export.files["normal"] = normal_path
            if os.environ.get(UE5_NORMAL_ENV_ONNX, "").strip():
                export.warnings.append(
                    "normal.png generado con fallback procedural; ruta ONNX validada pero inferencia normal ONNX pendiente."
                )
                sources["normal"] = "procedural_luminance_fallback_with_onnx_candidate"
            else:
                export.warnings.append("normal.png generado con fallback procedural por luminancia, no IA.")
                sources["normal"] = "procedural_luminance_fallback"
        except Exception as exc:
            export.warnings.append(f"normal.png omitido: {exc}")
    else:
...
def _save_image_variant(image: Image.Image, target_path: Path, fmt: str, jpeg_quality: int = 95):
    ...
    if fmt == "jpg":
        ...
        svg = (
            ...
            f'  <title>Image Enhancer export {width}x{height}</title>\n'
            f'  <image width="{width}" height="{height}" href="data:image/png;base64,{png_data}" />\n'
            '</svg>\n'
        )
...
def save_image_variant(image: Image.Image, target_path: Path, fmt: str, jpeg_quality: int = 95):
    _save_image_variant(image, target_path, fmt, jpeg_quality)
...
def run_svg_export_smoke() -> int:
    """Valida export SVG compatible con PNG incrustado."""
    try:
        with tempfile.TemporaryDirectory(prefix="image_enhancer_svg_smoke_") as tmp_dir:
            root = Path(tmp_dir)
            image = Image.new("RGBA", (24, 16), (80, 120, 210, 220))
            image.putpixel((4, 4), (240, 120, 40, 255))

            direct_svg = root / "direct.svg"
            save_image_variant(image, direct_svg, "svg")
            svg_text = direct_svg.read_text(encoding="utf-8")
            if "<svg" not in svg_text or "data:image/png;base64," not in svg_text:
                raise RuntimeError("SVG directo no contiene la imagen PNG incrustada esperada")
            ...
            if len(outputs) != 1 or outputs[0].suffix.lower() != ".svg":
                raise RuntimeError("Export profile SVG no devolvio un unico archivo .svg")
            if not outputs[0].exists():
                raise RuntimeError("Export profile SVG no genero archivo de salida")

        print("OK: SVG export smoke passed")
        return 0
    except Exception as exc:
        print(f"FAIL: SVG export smoke failed: {exc}", file=sys.stderr)
        return 1
...
def run_ue5_depth_normal_smoke() -> int:
    """Valida export UE5 con depth ONNX real y normal procedural fallback."""
    MODULE_REGISTRY.activate("ue5_depth")
    MODULE_REGISTRY.activate("ue5_normal_map")
    try:
        img = Image.new("RGBA", (64, 48), (92, 84, 70, 255))
        draw = ImageDraw.Draw(img)
        draw.rectangle([6, 8, 30, 36], fill=(180, 130, 82, 255))
        draw.ellipse([34, 10, 58, 38], fill=(80, 120, 190, 255))
        metrics = collect_basic_metrics(
            img,
            source_size=img.size,
            provider="ue5-depth-normal-smoke",
            warnings=[],
            active_modules=("ue5_texture_set", "ue5_depth", "ue5_normal_map"),
            render_mode="final",
        )
        with tempfile.TemporaryDirectory(prefix="image_enhancer_ue5_depth_normal_") as tmp_dir:
            export = export_ue5_texture_set(
                img,
                Path(tmp_dir),
                "Depth Normal Smoke",
            ...
            depth_path = export.folder / "depth.png"
            normal_path = export.folder / "normal.png"
            manifest_path = export.folder / "texture_manifest.json"
            if not depth_path.exists():
                raise RuntimeError("depth.png no fue exportado")
            if not normal_path.exists():
                raise RuntimeError("normal.png no fue exportado")
            with Image.open(depth_path) as depth_img:
                if depth_img.mode != "L" or depth_img.size != img.size:
                    raise RuntimeError(f"depth.png inesperado: {depth_img.mode} {depth_img.size}")
            with Image.open(normal_path) as normal_img:
                if normal_img.mode != "RGB" or normal_img.size != img.size:
                    raise RuntimeError(f"normal.png inesperado: {normal_img.mode} {normal_img.size}")
            manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
            files = manifest.get("files", {})
            if files.get("depth") != "depth.png" or files.get("normal") != "normal.png":
                raise RuntimeError("manifest no registra depth/normal")
        print("OK: UE5 depth/normal smoke passed (Depth Anything ONNX + normal procedural)")
        return 0
    except Exception as exc:
        print(f"FAIL: UE5 depth/normal smoke failed: {exc}", file=sys.stderr)
        return 1
...
def run_ue5_candidates_smoke() -> int:
    ...
    ok = True
    if os.environ.get(UE5_DEPTH_ENV_ONNX, "").strip():
        ok = _print_optional_file_probe("UE5 Depth ONNX", UE5_DEPTH_ENV_ONNX, require_ort=True) and ok
    elif DEPTH_ANYTHING_ONNX_PATH.exists() and DEPTH_ANYTHING_DATA_PATH.exists():
        print(f"OK: UE5 Depth ONNX candidato cacheado -> {DEPTH_ANYTHING_ONNX_PATH}")
    else:
        print("OK: UE5 Depth ONNX descargable bajo demanda (Depth Anything v2 Small q4f16)")

    ok = _print_optional_file_probe("UE5 Normal ONNX", UE5_NORMAL_ENV_ONNX, require_ort=True) and ok
    if not os.environ.get(UE5_NORMAL_ENV_ONNX, "").strip():
        print("OK: UE5 Normal Map usa fallback procedural por luminancia si el modulo esta activo")
    if ok:
...
def run_quality_pyiqa_smoke() -> int:
    """Valida un backend PyIQA real; falla si solo existe fallback."""
    try:
        img = Image.new("RGBA", (128, 128), (122, 134, 148, 255))
        draw = ImageDraw.Draw(img)
        draw.rectangle([16, 16, 96, 96], fill=(180, 190, 200, 255))
        metrics = collect_basic_metrics(
            img,
            source_size=img.size,
            provider="quality-smoke",
            warnings=[],
            active_modules=("quality_pyiqa",),
            render_mode="final",
        )
        score = metrics.get("score", "")
        if not score:
            raise RuntimeError(
                "PyIQA no disponible. Instala pyiqa en un runtime externo o define "
                "IMAGE_ENHANCER_PYIQA_PYTHON."
            )
        print(f"OK: PyIQA backend smoke passed ({score})")
        return 0
    except Exception as exc:
        print(f"FAIL: PyIQA backend smoke failed: {exc}", file=sys.stderr)
        return 1
