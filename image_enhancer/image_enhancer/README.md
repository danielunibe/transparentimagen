...
- Exportar SVG compatible con la imagen PNG incrustada para conservar fidelidad y transparencia.
...
Validacion de export SVG compatible:
...
dist\ImageEnhancer-Unova\ImageEnhancer-Unova.exe --smoke-modules
dist\ImageEnhancer-Unova\ImageEnhancer-Unova.exe --smoke-ue5-depth-normal
```

`construir_exe.bat` recolecta el paquete `nvidia` y falla el build si el EXE no pasa `--smoke-cuda`.
El EXE reconstruido no empaqueta `.venv_pyiqa`, `pyiqa`, `torch` ni modelos descargados en `%USERPROFILE%\.image_enhancer_models`.
...
21. Guardar como PNG con transparencia, SVG compatible con PNG incrustada o JPG con fondo blanco.
...
- Fase 10: ComfyUI externo opcional integrado con smoke API, workflow JSON, retorno no bloqueante y soporte para reference/control images; no es requisito del nucleo.
- Fase 11: separado en `..\..\VIDEO_SPRINT.md`; no entra en el runtime actual.

Siguiente orden recomendado:

1. Validar un ONNX real para `IMAGE_ENHANCER_UE5_NORMAL_ONNX` si se quiere reemplazar el normal procedural.
2. Profundizar Fase 7/8 solo con modelos externos/ONNX ya probados.
3. Levantar ComfyUI real solo si se necesita un modo creativo avanzado tipo img2img/IP-Adapter/ControlNet.
...
- ComfyUI es opcional: la app no depende de el para recorte, upscale, batch, calidad ni export UE5.
