...
REALESRGAN_SCALE = 4
DEPTH_ANYTHING_RELEASE = "hf-q4f16"
...
DEPTH_ANYTHING_DATA_URL = (
    "https://huggingface.co/onnx-community/depth-anything-v2-small-ONNX/"
    "resolve/main/onnx/model_q4f16.onnx_data"
)
DEPTH_ANYTHING_CACHE_DIR = (
    Path.home() / ".image_enhancer_models" / "depth_anything_v2_small_onnx" / DEPTH_ANYTHING_RELEASE
)
DEPTH_ANYTHING_ONNX_PATH = DEPTH_ANYTHING_CACHE_DIR / "model_q4f16.onnx"
DEPTH_ANYTHING_DATA_PATH = DEPTH_ANYTHING_CACHE_DIR / "model_q4f16.onnx_data"
...
SWINIR_ENV_ONNX = "IMAGE_ENHANCER_SWINIR_ONNX"
NAFNET_ENV_ONNX = "IMAGE_ENHANCER_NAFNET_ONNX"
...
def resolve_pyiqa_python_path() -> Path | None:
    env_path = os.environ.get(PYIQA_ENV_PYTHON, "").strip()
    if env_path:
        path = Path(env_path).expanduser()
        return path if path.exists() else None

    local_runtime = Path(__file__).resolve().parent / ".venv_pyiqa" / "Scripts" / "python.exe"
...
class ModuleRegistry:
    def __init__(self):
        ...
        self._specs: dict[str, ModuleSpec] = {
            ...
            "ue5_texture_set": ModuleSpec(
                ...
                description="Export profile local con carpeta estable, albedo.png, depth.png, normal.png y manifest tecnico.",
                fallback="albedo + manifest si faltan modulos de mapas",
            ...
            "ue5_depth": ModuleSpec(
                ...
                family="UE5 Texture Tools",
                description="Mapa de profundidad local para export tecnico hacia Unreal Engine.",
                dependencies=(UE5_DEPTH_ENV_ONNX,),
                model_required="depth_anything_v2.onnx",
                fallback="sin depth map",
                smoke_test="--smoke-module ue5_depth",
                implemented=True,
            ),
            "ue5_normal_map": ModuleSpec(
                id="ue5_normal_map",
                name="Normal Map procedural",
                family="UE5 Texture Tools",
                description="Normal map procedural por luminancia para sets PBR; ONNX real queda como ruta futura.",
                dependencies=(UE5_NORMAL_ENV_ONNX,),
                fallback="sin normal map si falta NumPy",
            ...
            "restoration_swinir": ModuleSpec(
                ...
                dependencies=(SWINIR_ENV_ONNX,),
                fallback="Real-ESRGAN o LANCZOS",
                smoke_test="--smoke-module restoration_swinir",
            ),
            "restoration_nafnet": ModuleSpec(
                id="restoration_nafnet",
                name="NAFNet denoise",
                family="Restauracion",
                description="Candidato ONNX opcional para ruido/artefactos cuando el caso real lo justifique.",
                dependencies=(NAFNET_ENV_ONNX,),
                model_required="nafnet.onnx",
                fallback="Luz/color local o Real-ESRGAN",
                smoke_test="--smoke-module restoration_nafnet",
            ),
    ...
    def status(self, module_id: str) -> str:
        ...
        if module_id == "portrait_gfpgan_external":
        ...
        if module_id == "ue5_depth":
            ...
            if model_path and Path(model_path).expanduser().exists():
                return MODULE_STATE_READY
            if DEPTH_ANYTHING_ONNX_PATH.exists() and DEPTH_ANYTHING_DATA_PATH.exists():
                return MODULE_STATE_READY
            return MODULE_STATE_FALLBACK
        if module_id == "ue5_normal_map":
            ...
            if model_path and Path(model_path).expanduser().exists():
                return MODULE_STATE_READY
            return MODULE_STATE_READY if NUMPY_AVAILABLE else MODULE_STATE_FALLBACK
        return MODULE_STATE_READY
    ...
    def validate(self, module_id: str) -> tuple[bool, str]:
        ...
        optional_paths = {
            ...
            "restoration_swinir": (SWINIR_ENV_ONNX, True),
            "restoration_nafnet": (NAFNET_ENV_ONNX, True),
        }
        ...
        if state == MODULE_STATE_FALLBACK:
            if module_id == "portrait_gfpgan_external":
                return True, (
                    "GFPGAN externo queda en fallback. Define IMAGE_ENHANCER_GFPGAN_SCRIPT "
                    "y opcionalmente IMAGE_ENHANCER_GFPGAN_PYTHON para activarlo sin tocar el EXE."
                )
            if module_id == "creative_comfyui":
                return True, (
            ...
            if module_id == "ue5_depth":
                if ORT_AVAILABLE and NUMPY_AVAILABLE:
                    return True, (
                        "ue5_depth puede descargar/cachear Depth Anything v2 Small ONNX q4f16 "
                        "fuera del EXE al activarse."
                    )
                return True, (
            ...
            if module_id == "ue5_normal_map":
                return True, (
                    "ue5_normal_map usa fallback procedural por luminancia. Define "
                    "IMAGE_ENHANCER_UE5_NORMAL_ONNX para validar un modelo local real."
                )
        ...
        if module_id == "quality_pyiqa":
            ...
            if external_python:
                return True, f"PyIQA listo via {external_python} con metrica '{metric_name}'."
            return True, f"PyIQA importable en este entorno con metrica '{metric_name}'."
        if module_id == "ue5_depth":
            return True, "Depth Anything v2 Small ONNX listo para exportar depth.png."
        if module_id == "ue5_normal_map":
            model_path = os.environ.get(UE5_NORMAL_ENV_ONNX, "").strip()
            if model_path and Path(model_path).expanduser().exists():
                return True, f"Normal map ONNX candidato listo: {Path(model_path).expanduser()}."
            return True, "Normal map procedural por luminancia listo; no es IA."
...
def _download_file(url: str, destination: Path, status_cb: Callable[[str, str], None] | None = None):
    destination.parent.mkdir(parents=True, exist_ok=True)
    tmp_path = destination.with_name(destination.name + ".tmp")
    try:
        req = urllib.request.Request(url, headers={"User-Agent": "ImageEnhancer-Unova/1.0"})
        with urllib.request.urlopen(req, timeout=120) as response:
            ...
            with tmp_path.open("wb") as fh:
                while True:
                    ...
                    now = time.monotonic()
                    if status_cb and total and now - last_report > 0.5:
                        pct = int(done * 100 / total)
                        _notify(status_cb, f"Descargando {destination.name}... {pct}%", TEXT_DIM)
                        last_report = now
        ...
        try:
            ...
            pass
        raise RuntimeError(f"No se pudo descargar {destination.name}.") from exc


def ensure_depth_anything_model(status_cb: Callable[[str, str], None] | None = None) -> Path:
    """Devuelve un Depth Anything v2 Small ONNX externo; descarga q4f16 si no hay ruta manual."""
    env_path = os.environ.get(UE5_DEPTH_ENV_ONNX, "").strip()
    if env_path:
        path = Path(env_path).expanduser()
        if path.exists():
            return path
        raise RuntimeError(f"{UE5_DEPTH_ENV_ONNX} apunta a una ruta inexistente: {path}")

    if not DEPTH_ANYTHING_ONNX_PATH.exists():
        _notify(status_cb, "Descargando Depth Anything v2 Small ONNX q4f16...", TEXT_DIM)
        _download_file(DEPTH_ANYTHING_MODEL_URL, DEPTH_ANYTHING_ONNX_PATH, status_cb)
    if not DEPTH_ANYTHING_DATA_PATH.exists():
        _notify(status_cb, "Descargando datos externos de Depth Anything v2...", TEXT_DIM)
        _download_file(DEPTH_ANYTHING_DATA_URL, DEPTH_ANYTHING_DATA_PATH, status_cb)
    return DEPTH_ANYTHING_ONNX_PATH
...
class ModelManager:
    ...
    def __init__(self):
        ...
        self._depth_anything_session = None
        self._depth_anything_model_path = None
    ...
    def get_depth_anything_session(self, status_cb: Callable[[str, str], None] | None = None):
        with self._lock:
            if not ORT_AVAILABLE:
                raise RuntimeError("onnxruntime no esta instalado. Ejecuta instalar.bat.")
            if not NUMPY_AVAILABLE:
            ...
            model_path = ensure_depth_anything_model(status_cb)
            if self._depth_anything_session is not None and self._depth_anything_model_path == model_path:
                return self._depth_anything_session

            _notify(status_cb, f"Cargando Depth Anything v2 en ONNX Runtime ({self.runtime_label()})...", TEXT_DIM)
            self._depth_anything_session = self._create_onnx_session(model_path, "Depth Anything v2")
            self._depth_anything_model_path = model_path
            return self._depth_anything_session
...
def get_depth_anything_session(status_cb: Callable[[str, str], None] | None = None):
    """Devuelve la sesion singleton de Depth Anything v2 ONNX."""
    return MODEL_MANAGER.get_depth_anything_session(status_cb)
