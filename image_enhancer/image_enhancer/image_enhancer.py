...
from pipeline import (
    ...
    run_modules_smoke,
    run_portrait_smoke,
    run_production_batch_smoke,
    run_quality_pyiqa_smoke,
    run_realesrgan_smoke,
    run_rembg_smoke,
    run_segmentation_compare_smoke,
    run_smoke,
    run_svg_export_smoke,
    run_ue5_candidates_smoke,
    run_ue5_depth_normal_smoke,
    run_ue5_texture_set_smoke,
)
from video_pipeline import run_video_sprint_cli, run_video_sprint_smoke
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--smoke-ue5-depth-normal",
        action="store_true",
        help="Validate UE5 depth.png and normal.png export using real/fallback modules.",
    )
    parser.add_argument(
        "--smoke-quality-pyiqa",
        action="store_true",
        help="Validate a real PyIQA backend; fails when only fallback is available.",
    )
    parser.add_argument(
        "--smoke-cuda",
        action="store_true",
    ...
    parser.add_argument(
        "--smoke-production-batch",
        action="store_true",
        help="Validate the production batch/presets/report flow without opening the UI.",
    )
    parser.add_argument(
        "--smoke-svg-export",
        action="store_true",
        help="Validate SVG export compatible with embedded PNG data.",
    )
    parser.add_argument(
        "--smoke-video-sprint",
        action="store_true",
        help="Validate the isolated video sprint pipeline without touching the image runtime.",
    )
    parser.add_argument(
        "--smoke-modules",
        action="store_true",
    ...
    parser.add_argument("--video-source", default="", help="Source video path for the isolated video sprint.")
    parser.add_argument("--video-output", default="", help="Output video path for the isolated video sprint.")
    parser.add_argument(
        "--video-mode",
        default="copy",
        choices=("copy", "light_restore"),
        help="Frame processor used by the isolated video sprint.",
    )
    parser.add_argument(
        "--video-keep-temp",
        action="store_true",
        help="Keep temporary extracted/processed frames for the isolated video sprint.",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.smoke_ue5_depth_normal:
        return run_ue5_depth_normal_smoke()
    if args.smoke_quality_pyiqa:
        return run_quality_pyiqa_smoke()
    if args.smoke_cuda:
        return run_cuda_smoke()
    if args.smoke_comfyui:
        return run_comfyui_smoke()
    if args.smoke_production_batch:
        return run_production_batch_smoke()
    if args.smoke_svg_export:
        return run_svg_export_smoke()
    if args.smoke_video_sprint:
        return run_video_sprint_smoke()
    if args.smoke_modules:
        return run_modules_smoke()
    if args.smoke_module:
        return run_module_smoke(args.smoke_module)
    if args.video_source and args.video_output:
        return run_video_sprint_cli(
            args.video_source,
            args.video_output,
            frame_processor=args.video_mode,
            keep_temp=args.video_keep_temp,
        )
    return run_app()
