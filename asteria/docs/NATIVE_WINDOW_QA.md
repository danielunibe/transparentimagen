...
## 2026-06-07 - Windows native persistence closure

Estado: `implemented_and_visually_verified`

- La ventana `target\debug\app.exe` fue capturada con la UI completa.
- El estado visible reporta `Tauri + SQLite`, `Native paths` y `SQLite Ready`.
- SQLite contiene `migration_log`, `folder_sources`, `kv_store` y `thumbnail_cache`.
- Los tests Rust validan carpeta legible, ruta inexistente, escaneo de medios y rechazo de extensiones no permitidas.
- `tauri-plugin-window-state` esta activo.
- `npm run tauri:build` genero:
  - `src-tauri\target\release\bundle\msi\Asteria_0.1.0_x64_en-US.msi`
  - `src-tauri\target\release\bundle\nsis\Asteria_0.1.0_x64-setup.exe`
- El sidecar aparece en `src-tauri\target\release\sidecars\python-ai`.
- No se completo una seleccion automatizada del dialogo de carpeta Windows; el contrato de escaneo quedo cubierto directamente por tests Rust.
- Python sigue siendo un prerrequisito externo del sidecar empaquetado.
