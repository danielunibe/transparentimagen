...
- Estado: `partially_done` el 2026-06-06.
...
- Resultado: ventana y bridge capturados; Material Vault carga. El sidecar falla desde UI por ruta relativa bajo `src-tauri` y no habia material PBR para ejercer preview/diagnostics.

## Phase 41B - Native Sidecar Path and PBR Fixture QA

- Objetivo: cerrar los dos bloqueos demostrados por Phase 41 sin entrar en packaging.
- Archivos probables: `src-tauri/src/lib.rs`, fixtures PBR temporales y docs de QA.
- Riesgos: mezclar la ruta de desarrollo con el contrato empaquetado o mutar datos reales.
- No hacer: no ejecutar `tauri build`, no descargar modelos, no ampliar permisos y no modificar PBR.
- Validaciones: gate completo, sidecar health/capabilities desde CLI, y ventana nativa observada si la sesion lo permite.
- Criterio de aceptacion: sidecar nativo operativo, fixtures PBR temporales listos y preview 3D ejercible sin mutar assets.
...
Siguiente fase ejecutable: Phase 41B - Native Sidecar Path and PBR Fixture QA.
