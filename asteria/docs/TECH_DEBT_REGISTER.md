...
| --- | --- | --- | --- | --- | --- | --- |
| TD-001 | Build / Vite | Main bundle remains larger than 500 kB | Medium | Slower startup and harder future splitting | Address only in a dedicated performance phase | Open |
| TD-002 | Type validation | Dedicated `tsconfig.check.json` remains separate from build | Low | Two gates must stay aligned | Keep both commands in validation | Accepted |
...
| TD-025 | Packaging | Python scripts are bundled but Python itself is external | High | A clean machine without Python/dependencies cannot process images | Package a managed Python runtime in a future delivery phase | Partially mitigated |
...
| TD-029 | Native sidecar | Script and model paths previously depended on the checkout | High | Installed runtime could fail | Release script resolves from Tauri resources; models resolve from app data | Resolved |
| TD-033 | Native media | Current native scan loads supported files into WebView memory | Medium-High | Very large folders can consume substantial RAM | Add lazy media loading/virtualization before watch folders | Open |
