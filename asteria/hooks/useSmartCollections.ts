...
export function useSmartCollections(
    ...
    const [storedVariants, setStoredVariants] = useState<AssetVariant[]>([]);
    const [activeCollectionId, setActiveCollectionId] = useState<string | null>(null);

    const builtInCollections = useMemo(() => getBuiltInSmartCollections(), []);

    useEffect(() => {
        void loadSavedViews().then(setSavedViews);
        void loadAllStoredVariants().then(setStoredVariants);
    }, []);
    ...
    }, [activeSessionVariants, aiJobs, exportJobs, storedVariants]);
    ...
    const saveCurrentView = useCallback((label: string, criteria: SmartCollection['criteria']) => {
        ...
        void saveSavedViews(newSavedViews);
    }, [savedViews]);

    const deleteSavedView = useCallback((id: string) => {
        void deleteSavedViewService(id).then(setSavedViews);
