import { deleteJsonRecord, isSqlitePersistenceAvailable, mirrorStorageRecord } from './persistenceService';
...
export function safeSetJson<T>(key: string, value: T, version: number = 1): void {
    ...
    try {
        ...
        if (isSqlitePersistenceAvailable()) {
            void mirrorStorageRecord(key, record);
        }
    } catch (e) {
...
export function safeRemove(key: string): void {
    ...
    try {
        ...
        if (isSqlitePersistenceAvailable()) {
            void deleteJsonRecord('localStorage', key);
        }
    } catch (e) {
