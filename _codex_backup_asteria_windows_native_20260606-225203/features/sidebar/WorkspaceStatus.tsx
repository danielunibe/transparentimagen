import { Monitor, Globe, Server, HardDrive, Trash2 } from 'lucide-react';
import { useEffect, useState } from 'react';
import { getRuntimeCapabilities, RuntimeCapabilities } from '@/services/runtimeService';
import { isIndexedDbAvailable } from '@/services/indexedDbService';
import { clearThumbnailCache } from '@/services/thumbnailService';
import { isSqlitePersistenceAvailable } from '@/services/persistenceService';
...
export function WorkspaceStatus() {
    ...
    useEffect(() => {
        ...
        setIdbReady(isIndexedDbAvailable());
        setSqliteReady(isSqlitePersistenceAvailable());
    }, []);
    ...
    let title = "Web Preview";
    let icon = <Globe className="w-4.5 h-4.5 text-[#b8b59f]" strokeWidth={1.5} />;
    ...
    if (caps) {
        if (caps.runtime === 'tauri') {
            ...
            icon = <Monitor className="w-4.5 h-4.5 text-[#b8b59f]" strokeWidth={1.5} />;
        } else if (caps.canUseFileSystemAccess && idbReady) {
            title = "Local Browser + IDB";
            icon = <Server className="w-4.5 h-4.5 text-[#b8b59f]" strokeWidth={1.5} />;
        } else if (caps.canUseFileSystemAccess) {
             title = "Local Browser";
             icon = <Server className="w-4.5 h-4.5 text-[#b8b59f]" strokeWidth={1.5} />;
        }
    }
                ...
                <div className="flex items-center gap-3 mb-3">
                    ...
                    <div className="flex flex-col">
                        ...
                        <span className="text-[10px] text-[#7f826f] font-medium mt-0.5 leading-tight flex items-center gap-1.5">
                            {caps?.canUseFileSystemAccess ? 'File Handles' : 'Files Limited'}
                            <span className="opacity-40">•</span>
                            {idbReady ? 'IDB Ready' : 'No IDB'}
                        </span>
                    </div>
                </div>
