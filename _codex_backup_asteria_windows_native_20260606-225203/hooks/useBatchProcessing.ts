...
export function useBatchProcessing(items: GalleryItem[], queueExportJob?: (item: GalleryItem, recipe: ExportRecipe, activeVariant?: AssetVariant) => void) {
    ...
    const [processingReports, setProcessingReports] = useState<ProcessingReport[]>(() => loadProcessingReports());

    useEffect(() => {
        const stored = safeGetJson<BatchProcessingJob[]>(BATCH_JOBS_STORAGE_KEY, []);
        if (stored.length > 0) {
            setBatchJobs(stored);
        }
    }, []);
    ...
    const saveJobs = useCallback((jobs: BatchProcessingJob[]) => {
        ...
        safeSetJson(BATCH_JOBS_STORAGE_KEY, toSave);
    }, []);
    ...
    const runBatchJob = useCallback(async (job: BatchProcessingJob) => {
        ...
        for (const itemId of job.itemIds) {
            ...
            try {
                ...
                if (variant) {
                    ...
                    const existing = loadVariants(item.id);
                    // Filter out original because loadVariants might return it depending on implementation, actually wait.
                    // loadVariants purely loads from localStorage. So it shouldn't contain 'original' unless we saved it.
                    saveVariants(item.id, [...existing, variant]);
        ...
        const savedReports = saveProcessingReport(report);
