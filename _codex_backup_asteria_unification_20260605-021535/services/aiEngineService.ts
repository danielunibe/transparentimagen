...
export function loadAiEngineSettings(): LocalAiEngineSettings {
    if (typeof window === 'undefined') return getDefaultAiEngineSettings();
    try {
        const stored = localStorage.getItem(AI_ENGINE_SETTINGS_KEY);
        if (stored) {
    ...
    return getDefaultAiEngineSettings();
}

export function saveAiEngineSettings(settings: LocalAiEngineSettings): void {
...
export async function getActiveEngine(): Promise<LocalAiEngineInfo> {
    const settings = loadAiEngineSettings();
    ...
    return {
        ...
        status: 'unavailable',
        capabilities: {
            enhance: false, removeBg: false, portrait: false, ue5: false, promptEdit: false, returnsImage: false, supportsProgress: false, supportsCancel: false
        }
    };
}
