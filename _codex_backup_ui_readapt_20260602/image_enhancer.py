...
class ImageEnhancerApp:
    ...
    def _build_ui(self):
        # ─ Top app bar ───────────────────────────────────────────────────
        topbar = tk.Frame(self.root, bg=TOPBAR_BG, height=58)
        topbar.pack(fill="x")
        topbar.pack_propagate(False)

        brand = tk.Frame(topbar, bg=TOPBAR_BG)
        brand.pack(side="left", padx=20, fill="y")
        tk.Label(
            brand, text="IMAGE ENHANCER",
            font=("Consolas", 14, "bold"), fg=TEXT, bg=TOPBAR_BG
        ).pack(side="left")
        tk.Label(
            brand, text="  Unova Asset Studio",
            font=("Consolas", 9), fg=TEXT_DIM, bg=TOPBAR_BG
        ).pack(side="left")

        top_status = tk.Frame(topbar, bg=TOPBAR_BG)
        top_status.pack(side="right", padx=20)
        self._top_chip(top_status, "u2net default", SUCCESS)
        self._top_chip(top_status, "ONNX CPU", TEXT_DIM if ORT_AVAILABLE else WARNING)
        if not REMBG_AVAILABLE:
            self._top_chip(top_status, "rembg falta", WARNING)
        elif not CV2_AVAILABLE:
            self._top_chip(top_status, "cv2 limitado", WARNING)
        ...
        sidebar = tk.Frame(shell, bg=SIDEBAR_BG, width=220, padx=14, pady=14)
        ...
        workspace_top = tk.Frame(workspace, bg=BG)
        workspace_top.pack(fill="x", pady=(0, 10))
        tk.Label(
            workspace_top, text="Image Workspace",
            font=("Consolas", 16, "bold"), fg=TEXT, bg=BG
        ).pack(side="left")
        tk.Label(
            workspace_top, text="  Recorte, bordes, upscale y export",
        ...
        inspector = tk.Frame(body, bg=PANEL, width=318)
    ...
    def _build_sidebar(self, parent):
        logo = tk.Frame(parent, bg=CARD_BG, height=54, highlightthickness=1, highlightbackground=BORDER)
        logo.pack(fill="x", pady=(0, 14))
        logo.pack_propagate(False)
        tk.Label(
            logo, text="IE",
            font=("Consolas", 16, "bold"), fg=CTA, bg=CARD_BG,
            width=4
        ).pack(side="left", padx=(10, 4))
        brand = tk.Frame(logo, bg=CARD_BG)
        brand.pack(side="left", fill="both", expand=True)
        tk.Label(
            brand, text="AI Studio",
            font=("Consolas", 9, "bold"), fg=TEXT, bg=CARD_BG,
            anchor="w"
        ).pack(fill="x", pady=(10, 0))
        tk.Label(
            brand, text="Local workflow",
        ...
        tk.Label(
            parent, text="FLUJO",
            font=("Consolas", 8, "bold"), fg=TEXT_DIM, bg=SIDEBAR_BG
        ).pack(anchor="w", pady=(2, 8))
        for idx, label, note in (
            ("01", "Imagen", "Cargar asset"),
            ("02", "Fondo", "u2net / birefnet"),
            ("03", "Bordes", "despill + pincel"),
            ("04", "Calidad", "LANCZOS / ESRGAN"),
            ("05", "Export", "PNG / JPG"),
        ):
            self._sidebar_step(parent, idx, label, note)

        tk.Frame(parent, bg=SIDEBAR_BG).pack(fill="both", expand=True)

        stats = tk.Frame(parent, bg=CARD_BG, padx=10, pady=10, highlightthickness=1, highlightbackground=BORDER)
        stats.pack(fill="x")
        tk.Label(
            stats, text="RUNTIME",
        ...
        self._runtime_row(stats, "preview", "A/B", CTA)

    def _sidebar_step(self, parent, idx: str, label: str, note: str):
        row = tk.Frame(parent, bg=SIDEBAR_BG, padx=8, pady=7)
        row.pack(fill="x", pady=(0, 4))
        tk.Label(
            row, text=idx,
            font=("Consolas", 8, "bold"), fg=CTA, bg=SIDEBAR_BG,
            width=4, anchor="w"
        ).pack(side="left")
        text = tk.Frame(row, bg=SIDEBAR_BG)
        text.pack(side="left", fill="x", expand=True)
        tk.Label(
            text, text=label,
            font=("Consolas", 9, "bold"), fg=TEXT, bg=SIDEBAR_BG,
            anchor="w"
        ).pack(fill="x")
        tk.Label(
            text, text=note,
            font=("Consolas", 7), fg=TEXT_DIM, bg=SIDEBAR_BG,
            anchor="w"
        ).pack(fill="x")
    ...
    def _runtime_row(self, parent, label: str, value: str, color: str):
        ...
        row.pack(fill="x", pady=(0, 4))
        tk.Label(row, text=label, font=("Consolas", 7), fg=TEXT_DIM, bg=CARD_BG).pack(side="left")
        tk.Label(row, text=value, font=("Consolas", 7, "bold"), fg=color, bg=CARD_BG).pack(side="right")
    ...
    def _build_left_panel(self, parent):
        ...
        self._section_label(parent, "BORDES AUTOMATICOS")
        ...
        tk.Label(mode_row, text="  Modo   :", font=("Consolas", 7), fg=TEXT_DIM, bg=PANEL).pack(side="left")
        ...
        tk.Label(engine_row, text="  Motor  :", font=("Consolas", 7), fg=TEXT_DIM, bg=PANEL).pack(side="left")
        ...
        self._section_label(parent, "FACE RECOVERY")
    ...
    def _build_action_bar(self, parent):
        ...
        self.action_file_lbl = tk.Label(
            ...
            font=("Consolas", 8, "bold"),
            anchor="w", width=22)
        self.action_file_lbl.pack(side="left", padx=(0, 8), fill="x")

        chips = tk.Frame(bar, bg=ACTION_BG)
        chips.pack(side="left", fill="x", expand=True)
        self._make_action_chip(chips, "model", f"Modelo {DEFAULT_MODEL_KEY}")
        self._make_action_chip(chips, "mode", "High Fidelity")
        self._make_action_chip(chips, "engine", "LANCZOS")
        self._make_action_chip(chips, "factor", "2x")
        ...
        self.refine_btn = tk.Button(
            ...
            state="disabled")
        self.refine_btn.pack(side="left", padx=(8, 4))

        self.save_btn = tk.Button(
            bar, text="Guardar PNG",
    ...
    def _refresh_action_chips(self):
        if hasattr(self, "action_file_lbl"):
            if self.input_path:
                ...
                self.action_file_lbl.configure(text="Sin imagen", fg=TEXT_DIM)
    ...
    def _load_image(self):
        ...
        try:
            ...
            self.process_btn.configure(state="normal", bg=CTA, fg=CTA_TEXT, text="PROCESAR")
    ...
    def _start_processing(self):
        ...
        self.process_btn.configure(state="disabled", bg=ACTION_BR, fg=TEXT_DIM, text="Procesando...")
        self.save_btn.configure(state="disabled", bg=ACTION_LOW, fg=TEXT_DIM)
        self.refine_btn.configure(state="disabled", bg=ACTION_LOW, fg=TEXT_DIM)
    ...
    def _on_error(self):
        ...
        self.process_btn.configure(state=state, bg=bg, fg=fg, text="PROCESAR")
