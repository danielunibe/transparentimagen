...
- Acciones secundarias: `Preview 25%`, `Refine Edge`, `Save PNG`, `UE5 Set` y `ComfyUI`.
...
- Decision visual/UX: `CALIDAD` vive en el inspector derecho como bloque compacto de metricas, antes de `MODULOS IA`, para revisar resultado sin invadir el visor central.
- Decision visual/UX: `UE5 Set` vive en la barra inferior y solo se habilita con render final; exporta una carpeta tecnica sin ocupar el flujo principal de guardado PNG/JPG.
- Decision visual/UX: Fase 7/8 se expresa como tarjetas activables (`Detector facial local`, `Luz/color local`) dentro de `MODULOS IA`, sin agregar paneles nuevos ni ocultar `Generate`.
- Decision operativa confirmada: `Luz/color local` es una capa de restauracion segura para fotos oscuras/planas; Zero-DCE, DeepWB y SwinIR quedan como candidatos futuros sin descarga automatica.
- Decision visual/UX: `PRODUCCION` vive en el inspector derecho como bloque operativo con preset guardado, export profile, batch y reporte; no invade el visor central.
- Decision visual/UX: `HISTORIAL` vive como lista compacta en el inspector derecho para revisar ultimos resultados sin abrir otra ventana.
- Decision visual/UX: `COMFYUI` se integra como boton y panel tecnico secundarios; solo habilita backend externo opcional y no cambia la identidad del flujo principal.
...
| Panel `CALIDAD` | Muestra metricas basicas sin autoajustar el pipeline | protegido |
...
| Boton `UE5 Set` | Export tecnico UE5 disponible solo tras render final | protegido |
| Bloque `PRODUCCION` | Da continuidad de lote/presets sin romper el flujo unitario principal | protegido |
| Boton `ComfyUI` | Puerta opcional al backend creativo externo sin desplazar `Generate` | protegido |
| Bloque `HISTORIAL` | Resume resultados recientes sin abrir mas navegacion | protegido |
| Tarjeta `Detector facial local` | Permite crop/mezcla de CodeFormer sin cambiar el layout | protegido |
| Tarjeta `Luz/color local` | Primer modulo operativo de restauracion sin dependencias nuevas | protegido |
